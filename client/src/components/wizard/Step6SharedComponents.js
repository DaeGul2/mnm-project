...
function HtmlInterpretationEditor({
  ...
  compact,
  previewPayload, // 🔹 추가
  autoConfig, // 🔹 GPT 자동 해석용 설정 { projectName, stageName, section, groupName, projectToken }
}) {
  ...
  const handleAutoGenerate = async () => {
    if (!autoConfig || !autoConfig.projectToken) {
      alert("프로젝트 토큰 정보가 없어 자동 해석을 호출할 수 없습니다.");
      return;
    }
    if (!previewPayload) {
      alert("해석에 사용할 데이터가 없습니다.");
      return;
    }

    const { projectName, stageName, section, groupName, projectToken } =
      autoConfig;

    try {
      setIsLoading(true);

      const res = await gptService.generateStep6Interpretation(
        {
      ...
      console.error("[STEP6 AUTO INTERPRETATION CLIENT ERROR]", err);
      alert("자동 해석 생성 중 오류가 발생했습니다.");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {autoConfig && !readOnly && (
            <button
              ...
              style={{
                ...
                backgroundColor: isLoading ? "#eef2ff" : "#4f46e5",
